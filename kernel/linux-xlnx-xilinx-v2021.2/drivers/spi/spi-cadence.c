...
#define CDNS_SPI_CR	0x00 /* 配置寄存器, RW */
				       ...
				       struct spi_transfer *transfer)
{
	...
	//printk("xspi->ref_clk is %u\r\n",xspi->ref_clk);

	//printk("frequency is %u\r\n",frequency);

	//printk("transfer->speed_hz is %u\r\n",transfer->speed_hz);

	//printk("xspi->speed_hz is %u\r\n",xspi->speed_hz);
	...
	if (xspi->speed_hz != transfer->speed_hz) {
		...
		//printk("baud_rate_val is %d\r\n",baud_rate_val);
		xspi->speed_hz = frequency / (2 << baud_rate_val);
		//printk("cdns_spi_config_clock_freq xspi->speed_hz is %u\r\n",xspi->speed_hz);
	}
	cdns_spi_write(xspi, CDNS_SPI_CR, ctrl_reg);
}
...
static int cdns_spi_probe(struct platform_device *pdev)
{
	...
	//printk("master->max_speed_hz is %u\r\n",master->max_speed_hz);
